...
export default {
  ...
  mounted() {
    ...
    // Preparing date from dummy
    let _dummy: {[key: string]: any} = {};
    dummy.forEach(d=>_dummy[d.date] = d);
    ...
    months.forEach((days, i) => {
      ...
      for (let day = 1; day <= days; day++) {
        ...
        for (let year = 2019; year <= 2021; year++) {
          ...
          let dummyItem = _dummy[dateStr] || {date: dateStr, crowdingAt8: null, congestionAt21And23: null, crowdingAt15: null, numberOfNewPositiveCases: 0};
    ...
    let volumeSeries = chart.series.unshift(am5xy.ColumnSeries.new(root, {
      name: "Number Of New Positive Cases",
    ...
    let sbBottom = chart.bottomAxesContainer.children.push(am5.Scrollbar.new(root, {
      orientation: "horizontal"
    }));

    chart.set("scrollbarX", sbBottom);
    chart.set("scrollbarY", sbRight);


    lineSerieses.forEach(ls=>ls.data.setAll(data));
